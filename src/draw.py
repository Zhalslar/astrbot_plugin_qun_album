...
def _find_font(bold: bool) -> Path | None:
    ...
    if FONT_DIR is not None:
        for ext in (".ttf", ".otf"):
            candidates.append(
                FONT_DIR / f"NotoSansSC-{'Bold' if bold else 'Regular'}{ext}"
            )

    for ext in (".ttf", ".otf"):
        candidates.append(
            RESOURCES_DIR / "fonts" / f"NotoSansSC-{'Bold' if bold else 'Regular'}{ext}"
        )

    opposite = "Bold" if not bold else "Regular"
    for ext in (".ttf", ".otf"):
        candidates.append(RESOURCES_DIR / "fonts" / f"NotoSansSC-{opposite}{ext}")
...
def draw_rounded_rectangle(
    ...
    draw.rounded_rectangle(
        (x1, y1, x2, y2), radius=corner_radius, fill=fill, outline=outline
    )
...
def render_my_friend(
    ...
    if show_title:
        ...
        buffer_w = 40
        buffer_h = 40
        lv_temp_img = Image.new(
            "RGBA", (lv_w + buffer_w, lv_h + buffer_h), (0, 0, 0, 0)
        )
        ...
        lv_temp_draw.text(
            (buffer_w // 2 - p_bbox[0], p_visual_top - p_bbox[1]),
            lv_prefix,
            font=lv_prefix_font,
            fill="white",
        )
        lv_temp_draw.text(
            (buffer_w // 2 + p_w + 4 - n_bbox[0], n_visual_top - n_bbox[1]),
            lv_num,
            font=lv_num_font,
            fill="white",
        )
        ...
        if final_title:
            ...
            if Pilmoji:
                ...
                with Pilmoji(
                    title_img, emoji_position_offset=(0, emoji_offset_y)
                ) as pilmoji:
                    pilmoji.text(
...
async def generate_single_meme(
    bot, user_id: str, text: str, info: dict, show_title: bool = True
) -> bytes | None:
    """获取头像并生成单张表情包"""
...
async def generate_meme(
    event: AiocqhttpMessageEvent, show_title: bool = True
) -> bytes | None:
    """处理消息事件并生成单张表情包"""
    ...
    return await generate_single_meme(
        event.bot, replyer_id, text, info, show_title=show_title
    )


async def generate_stitched_meme(
    event: AiocqhttpMessageEvent, messages: list[dict], show_title: bool = True
) -> bytes | None:
    """处理多条消息并生成垂直拼接的表情包"""
    images = []
    group_id = int(event.get_group_id())
    ...
    for msg in messages:
        ...
        img_bytes = await generate_single_meme(
            event.bot, user_id, text, info, show_title=show_title
        )
        if img_bytes:
